...
export default function Message({ user, setSelectedUser }: any) {
  const primary = {
    bgColor: "bg-white",
    titleColor: "",
  ...
  return (
    <div
      className={`message p-4 w-full flex flex-row ${theme.bgColor}`}
      onClick={(e: any) => {
        handleClick(e);
      }}
    >
      ...
      <div className="meta flex flex-row ml-2 pb-3 justify-between w-4/5	border-b">
        <div className="info">
          <h1 className={`name ${theme.titleColor}`}>{user.username}</h1>
          <span className={`excerpt text-sm ${theme.excerptColor}`}>
            Hello hi how are yuh
          </span>
        </div>
        <div className="flex flex-col">
          <span className={`stamp text-sm ${theme.titleColor}`}>8:00</span>
